...
export default function GroceriesAppContainer() {
	...
	// two new lists: one evolved product list and one filtered list
	const [evolvedProductList, setEvolvedProductList] = useState([]); // NEW evolved list
	...
	// 3. Filter Effect (Runs when activeFilter or evolvedProductList changes)
	useEffect(() => {
		if (activeFilter === "all") {
			setProductList(evolvedProductList);
			return;
		}

		const maxPrice = parseFloat(activeFilter);

		const filtered = evolvedProductList.filter((product) => {
	...
	}, [activeFilter, evolvedProductList]);
	...
	const handleProductsFromDB = async () => {
		try {
			...
			setEvolvedProductList(result.data);
	...
	const handleUpdateProduct = async (productId, updatedFormData) => {
		try {
			await axios.patch(`http://localhost:3000/products/${productId}`, updatedFormData).then((result) => {
				setPostResponse(result.data); // Trigger refresh
			});
		} catch (e) {
			console.log(error.message);
		...
		<div>
			...
			<div className="GroceriesApp-Container">
				<div className="ProductsView">
					...
					<FilterPricesForm activeFilter={activeFilter} handleFilterPrices={handleFilterPrices} />
				</div>
